  ...
  constructor(
    ...
    onGet(forceReload = false): void {
      if (!this.wasFetched && !forceReload){
        this.http.get<Category[]>(`${this.constants.baseUrl}InventoryCategories`)
        .subscribe(
          res => {
    ...
    onCreate(model: Category): Observable<Category> {
      return this.http.post<Category>(`${this.constants.baseUrl}InventoryCategories`, model)
        .pipe(
          map(res => {
    ...
    onEdit(model: Category): Observable<Category> {
      return this.http.put<Category>(`${this.constants.baseUrl}InventoryCategories`, model)
        .pipe(
          map(res => {
    ...
    onDelete(id: number): Observable<number> {
      return this.http.delete<number>(`${this.constants.baseUrl}InventoryCategories/${id}`)
      .pipe(
        map(res => {
